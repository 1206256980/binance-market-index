...
public class IndexCalculatorService {
     ...
     * 
     * 【业务背景】
     * 这是一个"回顾性分析"功能，用于分析：如果使用某个时间点的入场条件（涨幅榜Top N），
     * 在过去若干小时的市场行情下，做空策略的表现如何。
     * 
     * 【核心设计 - 入场条件与追踪范围解耦】
     * 
     * 1. entryTime（入场时间）的作用：
     * - 仅用于确定"做空哪些币种"（该时刻涨幅榜 Top N）
     * - 仅用于确定"入场价格是多少"（该时刻的开盘价）
     * - 与追踪范围完全无关
     * 
     * 2. 追踪范围的计算（与 entryTime 无关）：
     * - 开始时间 = 当前整点 - monitorHours
     * - 结束时间 = 当前整点 + 最新5分钟K线（标记为"实时"）
     * 
     * 3. 盈亏计算：
     * - 每个快照时间点使用该时刻的历史价格作为"平仓价"
     * - 盈亏 = (入场价 - 平仓价) / 入场价 × 投入金额
     * - 入场价在整个追踪过程中保持【固定不变】
     * 
     * 【使用场景举例】
     ...
     * - 用户点击"15:00"那一行的追踪按钮
     * - 系统获取 15:00 时刻的涨幅榜 Top N 作为做空标的，15:00 开盘价作为入场价
     * - 追踪范围 = 2月3日 20:00 → 2月4日 20:00 + 最新实时价格
     * - 共生成 24 个整点快照 + 1 个实时快照
     * - 每个快照的盈亏都是相对于 15:00 入场价计算
     * 
     * 【设计意义】
     * - 允许用户选择不同的入场时间点，对比"如果当时入场，历史表现如何"
     * - 不需要等待未来数据，可以立即看到完整的追踪曲线
     * - 用于评估某个特定涨幅榜组合在历史行情下的回撤和盈利情况
     * 
     * ================================================================================
     * 
     * @param entryTimeStr 入场时间（用于确定做空币种和入场价格，格式: "yyyy-MM-dd HH:mm"）
     * @param rankingHours 涨幅榜周期（如24表示24小时涨幅榜）
     * @param topN         做空涨幅榜前N名
     * @param totalAmount  总投入金额（平均分配到每个币种）
     * @param monitorHours 监控小时数（追踪范围长度，如24表示看过去24小时）
     * @param timezone     用户时区
    ...
    public Map<String, Object> getHourlyTracking(String entryTimeStr, List<String> symbolList,
        ...
        // ========== 第三步：获取基准点(pivot)的价格 ==========
        // entryTime 是基准点，盈亏为0。我们需要获取基准点时刻的价格
        Map<String, Double> pivotPrices = getHistoricalPrices(entryTimeUtc, allSymbols);
        if (pivotPrices.isEmpty()) {
            Map<String, Object> result = new HashMap<>();
            result.put("error", "无法获取基准点价格数据");
            return result;
        }
        log.info("获取到基准点 {} 的价格，共 {} 个币种", entryTime, pivotPrices.size());
        ...
        for (int h = 0; h <= totalHours; h++) {
            ...
            snapshot.put("isPivot", isAtPivot);
            snapshot.put("isLatest", false);
        ...
        if (latestTimeLocal.isAfter(entryTime)) {
            ...
            if (!latestPrices.isEmpty()) {
                ...
                Map<String, Object> latestSnapshot = calculatePivotSnapshotProfit(
                        ...
                        false, false, true);
                latestSnapshot.put("snapshotTime", latestTimeLocal.toString().replace("T", " "));
                latestSnapshot.put("hoursFromPivot", ChronoUnit.MINUTES.between(entryTime, latestTimeLocal) / 60.0);
        ...
        strategy.put("monitorHours", monitorHours);
        result.put("strategy", strategy);

        // 返回做空的币种和基准点价格信息
        List<Map<String, Object>> entryCoins = topCoins.stream().map(coin -> {
            String symbol = (String) coin.get("symbol");
            Map<String, Object> info = new HashMap<>();
            info.put("symbol", symbol);
            info.put("pivotPrice", pivotPrices.get(symbol)); // 基准点价格
            info.put("change24h", coin.get("change"));
            return info;
        }).collect(Collectors.toList());
    ...
    /**
     * 计算基准点(pivot)模式下的快照盈亏
     * 
     * 【核心逻辑】
     * - 基准点之前：snapshot价格作为入场价，pivot价格作为平仓价
     * 盈亏 = (snapshot价格 - pivot价格) / snapshot价格
     * - 基准点：盈亏 = 0
     * - 基准点之后：pivot价格作为入场价，snapshot价格作为平仓价
     * 盈亏 = (pivot价格 - snapshot价格) / pivot价格
     */
    private Map<String, Object> calculatePivotSnapshotProfit(
            List<Map<String, Object>> topCoins,
            Map<String, Double> snapshotPrices,
            Map<String, Double> pivotPrices,
            double amountPerCoin,
            boolean isBeforePivot,
            boolean isAtPivot,
            boolean isAfterPivot) {
        ...
        for (Map<String, Object> coin : topCoins) {
            ...
            if (isAtPivot) {
                // 基准点：盈亏为0
                profitPercent = 0.0;
                entryPrice = pivotPrice;
                exitPrice = pivotPrice;
            } else if (isBeforePivot) {
                // 基准点之前：snapshot价格作为入场价，pivot价格作为平仓价
                // 做空逻辑：盈亏 = (入场价 - 平仓价) / 入场价
                entryPrice = snapshotPrice;
                exitPrice = pivotPrice;
                profitPercent = ((entryPrice - exitPrice) / entryPrice) * 100.0;
            } else {
                // 基准点之后：pivot价格作为入场价，snapshot价格作为平仓价
                entryPrice = pivotPrice;
     ...
     * 计算单个快照的盈亏
     */
    private Map<String, Object> calculateSnapshotProfit(List<Map<String, Object>> topCoins,
            Map<String, Double> exitPrices, double amountPerCoin) {
    ...
    public Map<String, Object> getPriceIndexData(String entryTimeStr, List<String> symbolList, int rankingHours,
        ...
        try {
            ...
            log.warn("批量查询价格失败: {}", e.getMessage());
        }

        log.info("📊 批量查询耗时: {}ms", System.currentTimeMillis() - perfStart);

        Map<String, Double>[] startPriceMaps = new Map[] { startPrices1d, startPrices3d, startPrices7d };
        Map<String, Double>[] maxPriceMaps = new Map[] { maxPrices1d, maxPrices3d, maxPrices7d };
        int[][] periods = { { 1, 24 }, { 3, 72 }, { 7, 168 } };
        ...
        for (String symbol : symbols) {
            ...
            // 计算各周期涨幅（直接从批量查询结果中取值，无需再查DB）
            for (int i = 0; i < periods.length; i++) {
                String prefix = periods[i][0] + "d";
                Double startPrice = startPriceMaps[i].get(symbol);
                ...
                if (startPrice != null && startPrice > 0) {
                    ...
                    Double maxHigh = maxPriceMaps[i].get(symbol);
